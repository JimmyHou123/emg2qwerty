...
from emg2qwerty.data import get_subset_loader
from torch.utils.data import DataLoader
from emg2qwerty.data import WindowedEMGDataset  # ✅ Add this import
...
def main(config: DictConfig):
    ...
    if train_dataset is None:
        log.error("Could not retrieve train_dataset. Check datamodule.setup().")
        return 

    fraction = 0.95
    train_subset = get_subset_loader(train_dataset, fraction, batch_size=config.batch_size)

    log.info(f"Using {fraction*100:.0f}% of training data for this run.")
